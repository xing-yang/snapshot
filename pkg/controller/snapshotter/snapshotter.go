...
type VolumeSnapshotter interface {
	...
	UpdateVolumeSnapshot(snapshotName string, status *[]crdv1.VolumeSnapshotCondition) (*map[string]string, bool, error)
...
func (vs *volumeSnapshotter) waitForPluginSnapshot(snapshotName string, snapshot *crdv1.VolumeSnapshot) error {
	...
	for {
		completed, err = plugin.DescribeSnapshot(&snapshotDataObj)
			...
			if err != nil {
				glog.Infof("Error occurred when creating snapshot %s. Error: %v", snapshotName, err)
			}
	...
	if completed {
		...
		// UpdateVolmeSnapshot
                ...
                _, _, err = vs.UpdateVolumeSnapshot(snapshotName, &snapConditions)
                if err != nil {
...
func (vs *volumeSnapshotter) getSnapshotCreateFunc(snapshotName string, snapshot *crdv1.VolumeSnapshot) func() error {
	...
	return func() error {
		...
		if len(volSnapshotObj.Status.Conditions) > 0 &&
			(volSnapshotObj.Status.Conditions[0].Type == crdv1.VolumeSnapshotConditionReady ||
			volSnapshotObj.Status.Conditions[0].Type == crdv1.VolumeSnapshotConditionError) {
			glog.Infof("getSnapshotCreateFunc: Snapshot %s is Ready.", snapshotName)
			return nil
		} else if len(volSnapshotObj.Status.Conditions) > 0 &&
                        volSnapshotObj.Status.Conditions[0].Type == crdv1.VolumeSnapshotConditionPending &&
                        (volSnapshotObj.Status.Conditions[0].Status == v1.ConditionTrue ||
			volSnapshotObj.Status.Conditions[0].Status == v1.ConditionUnknown) {
			...
			// otherwise create metadata and update VolumeSnapshot object.
			tags, bFoundVolumeSnapshotMetadata, err := vs.UpdateVolumeSnapshot(snapshotName, nil)
...
func (vs *volumeSnapshotter) UpdateVolumeSnapshot(snapshotName string, status *[]crdv1.VolumeSnapshotCondition) (*map[string]string, bool, error) {
	var snapshotObj crdv1.VolumeSnapshot
	...
	snapNameSpace, snapName, err := cache.GetNameAndNameSpaceFromSnapshotName(snapshotName)
	if err != nil {
		return nil, false, fmt.Errorf("Error getting namespace and name from VolumeSnapshot name %s: %v", snapshotName, err)
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("Error copying snapshot object %s object from API server: %v", snapshotName, err)
	}
	snapshotCopy, ok := objCopy.(*crdv1.VolumeSnapshot)
	if !ok {
		return nil, false, fmt.Errorf("Error: expecting type VolumeSnapshot but received type %T", objCopy)
	}

        // Look for VolumeSnapshot metadata ...
	var tags map[string]string
	var bFoundVolumeSnapshotMetadata bool = false
	if snapshotCopy.Metadata.Name != "" && snapshotCopy.Metadata.Namespace != "" {
		if snapshotCopy.Metadata.Labels != nil {
			_, ok := snapshotCopy.Metadata.Labels["Timestamp"]
			if ok {
				glog.Infof("Found Metadata Labels in VolumeSnapshot object: [%#v].", snapshotCopy.Metadata.Labels)
	...
	if bFoundVolumeSnapshotMetadata == false {
		...
		glog.Infof("UpdateVolumeSnapshot: Setting VolumeSnapshotData name in VolumeSnapshotSpec of VolumeSnapshot object")
			...
			//TODO(xyang): Should we return error here?
                        return nil, false, fmt.Errorf("Error getting VolumeSnapshotData for VolumeSnapshot %s", snapshotName)
                }
	...
	err = vs.restClient.Put().
		...
		Do().Into(&result)
	if err != nil {
		return nil, false, fmt.Errorf("Error updating snapshot object %s on the API server: %v", snapshotName, err)
	}
	...
	return &cloudTags, bFoundVolumeSnapshotMetadata, nil
}
